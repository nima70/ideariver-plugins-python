...
import subprocess  # For running isolated test commands
import sys

# Path to the JSON file that contains plugin paths
PATHS_JSON_FILE = "plugin_paths.json"

# Function to clear __pycache__ in the given plugin path
def clear_pycache(plugin_path):
    pycache_path = os.path.join(plugin_path, "__pycache__")
    if os.path.exists(pycache_path):
        print(f"Clearing cache: {pycache_path}")
        shutil.rmtree(pycache_path)

# Function to execute tests in an isolated subprocess
def run_tests_in_subprocess(plugin_path, test_file, test_function):
    print(f"Running {test_function} from {test_file} in {plugin_path}...")
    # Construct the pytest command to run a specific test function
    command = [
        sys.executable, "-m", "pytest", f"{test_file}::{test_function}"
    ]
    result = subprocess.run(command, cwd=plugin_path)
    if result.returncode != 0:
        print(f"Test {test_function} in {test_file} failed.")
    else:
        print(f"Test {test_function} in {test_file} passed.")

# Function to load manifest.json and execute the tests
def load_and_execute_tests(plugin_path):
    ...
    for test_info in test_info_list:
        ...
        if test_file and test_function:
            ...
            run_tests_in_subprocess(plugin_path, test_file, test_function)
        else:
            print(f"Incomplete test info for {plugin_path}")
    ...
    with open(PATHS_JSON_FILE, "r") as file:
        ...
        for plugin_path in plugin_paths:
            ...
            if os.path.exists(full_plugin_path):
                # Run the tests for the current plugin
                load_and_execute_tests(full_plugin_path)
            else:
